<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            @if ($SubInstitution->id)
                {{ __('Edit Sub-Institution') }}
            @else
                {{ __('Add Sub-Institution') }}
            @endif
        </h2>
    </x-slot>
    ...
    <x-crm-card>
        ...
        <form method="POST" action="" id="edit-SubInstitution">
            @csrf
            ...
            <div class="w-1/2 pr-2" style="float: left">
                    ...
                    <x-institution-dropdown id="institution_id" class="block mt-1 w-full" name="institution_id"
                        :selected_id="old('institution_id', $SubInstitution->institution_id)" />
            </div>

            <div class="flex flex-wrap -mx-3">
                <x-label for="name" :value="__('Sub-Institution Name')" />
                <x-input id="name" class="block mt-1 w-full" type="text" name="name"
                    :value="old('name', $SubInstitution->name)" required autofocus />
            </div>
            <div class="flex flex-wrap -mx-3">
                <x-label for="address" :value="__('Address')" />
                <x-input id="address" class="block mt-1 w-full" type="text" name="address"
                    :value="old('address', $SubInstitution->address)" />
            </div>
            <div class="flex flex-wrap -mx-3">
                <div class="w-1/2 pr-2">
                    <x-label for="mobile" :value="__('Phone')" />
                    <x-input id="phone" class="block mt-1 w-full" type="text" name="phone"
                        :value="old('phone', $SubInstitution->phone)" pattern="(0){1}[0-9]{9}" required />
                </div>
                <div class="w-1/2">
                    <x-label for="email" :value="__('Email')" />
                    <x-input id="email" class="block mt-1 w-full" type="email" name="email"
                        :value="old('email', $SubInstitution->email)" />
                </div>
            </div>
            <div class="flex flex-wrap -mx-3">
                <div class="w-1/3 pr-2">
                    ...
                    <x-provinces-dropdown id="province_id" class="block mt-1 w-full" name="province_id"
                        :selected_id="old('province_id', $SubInstitution->province_id)" />
                </div>
                <div class="w-1/3 pr-2">
                    <x-label for="status" :value="__('District')" />
                    <x-districts-dropdown id="district_id" class="block mt-1 w-full" name="district_id"
                        :selected_id="old('district_id', $SubInstitution->district_id)"
                        :province_id="old('province_id', $SubInstitution->province_id)" />
                </div>

                <div class="w-1/3">
                    <x-label for="status" :value="__('City')" />
                    <x-cities-dropdown id="city_id" class="block mt-1 w-full" name="city_id"
                        :selected_id="old('city_id', $SubInstitution->city_id)"
                        :district_id="old('district_id', $SubInstitution->district_id)" />
                </div>
            </div>
            <div>
